...
- **A leftover `task_notification` must not leave the managed agent running after every declared child has settled.** Live background Tasks still keep the parent `running` (upstream #3366). Once the last declared child is completed/failed/canceled, that notification settles the autonomous turn so Archive finished can take a finished create_agent reviewer.
