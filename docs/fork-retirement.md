...
- fork integration: `origin/internal/main` at `f946dd775` — the docs-only ledger refresh [ZGEnergy/paseo#134](https://github.com/ZGEnergy/paseo/pull/134) is the tip; no product-code delta landed on the integration branch this review period
...
- the resolver [ZGEnergy/paseo#136](https://github.com/ZGEnergy/paseo/pull/136) (two-parent merge `f958554ac`, first parent `f946dd775`, second parent `d636abd7a`) integrates the delta with the union resolution — upstream's 0.9.0-beta.2 workspace pins plus the fork's optional markdown-extension deps in the plugin manifest, and lockfile/Nix hash regenerated from the union manifests; conflict resolution and nothing else, no product-code delta, no capability affected
- review-period fork merges since the 2026-09-18 refresh: none; no new downstream product capability, nothing added or retired
