...
**4 waiting.** No capability is `upstream-candidate`: live upstream `main` at `3cc4ae28664493b2f1ad75a6a6ed1d318c76e944` does not yet provide every required behavior for any capability. No capability retired this period; one retired history entry exists (host KaTeX, superseded by the plugin surface).

Last reviewed: 2026-09-18

Evidence baseline:

- fork integration: `origin/internal/main` at `6be25b186` — upstream content integrated through live `main` via the 2026-09-18 downstream sync [ZGEnergy/paseo#131](https://github.com/ZGEnergy/paseo/pull/131) (merge `6be25b186`), plus governance repair [#132](https://github.com/ZGEnergy/paseo/pull/132) (merge `83b867351`) and the docs-only ledger pair [#127](https://github.com/ZGEnergy/paseo/pull/127)/[#130](https://github.com/ZGEnergy/paseo/pull/130); the automated sync PR [#129](https://github.com/ZGEnergy/paseo/pull/129) closed as merged-not-needed once [#131](https://github.com/ZGEnergy/paseo/pull/131) landed its payload
- fork upstream mirror: `origin/main` at `3cc4ae28664493b2f1ad75a6a6ed1d318c76e944`
- upstream: `getpaseo/paseo` `main` at `3cc4ae28664493b2f1ad75a6a6ed1d318c76e944` — the mirror is current with upstream as of this review, and the integration branch now contains it
- the automated sync PR [#129](https://github.com/ZGEnergy/paseo/pull/129) (head `main`, base `internal/main`) went MERGE_STATE_DIRTY on the un-integrated upstream delta; no `Downstream sync: true` resolver existed, so this run branched from `internal/main`, merged live `main`, resolved the conflicted paths semantically, and opened [#131](https://github.com/ZGEnergy/paseo/pull/131); it was rebuilt once from the post-[#132](https://github.com/ZGEnergy/paseo/pull/132) `internal/main` so the head contains the base tip, and the Upstream import merge workflow landed it after 20/20 green checks and a fresh provenance validation
- the fork upstream mirror moved from `6830c46e9` to `3cc4ae286` across the review period (22 commits: character-paced streaming replies restored via the `text-reveal`/`useRevealedText` pipeline replacing the deleted `word-stream` module (#5013), desktop daemon-management memory overhead (#5007), composer-focused chat find (#4991), npm plugin install with review-source updates (#4975), website Safari mockups (#4990), composer layout stability (#4973), plugin external links and workspace browsers (#4972), nested provider-subagent link preservation (#4970), plugin host discovery with host-targeted SDK clients (#4971), plugin host navigation (#4942), pending attachment display (#4958), PR-tab auto-open (#4956), chronological history search (#4945), native composer sizing (#4946), archived-workspace restore preserving committed changes (#4926), voice reply latency with internal prompt wrappers hidden (#4927), the 0.9.0-beta.1 release cut, and desktop CI hardening — zero commits touch `providers/omp`, `providers/claude`, or any assistant-markdown surface path, and the capability greps below were re-run against `3cc4ae286`)
...
- Upstream `main` at `3cc4ae286` retains the `16.3.9` support floor but contains no `optionDetails` decoding or propagation (re-verified 2026-09-18; zero matches in source).
