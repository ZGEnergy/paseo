...
**4 waiting.** No capability is `upstream-candidate`: live upstream `main` at `2c8e8a826810337492cc5a38bb0bbd705b6fb632` does not yet provide every required behavior for any capability. No capability retired this period; one retired history entry exists (host KaTeX, superseded by the plugin surface).

Last reviewed: 2026-09-21

Evidence baseline:

- fork integration: `origin/internal/main` at `ade68b458` — the resolver [ZGEnergy/paseo#139](https://github.com/ZGEnergy/paseo/pull/139) was the sole review-period fork merge, landed by Upstream import merge on 2026-09-21 (provenance validated, two-parent shape re-checked); it carried the clean integration of the upstream mirror that the auto sync pull request [ZGEnergy/paseo#138](https://github.com/ZGEnergy/paseo/pull/138) (head `main`, base `internal/main`, MERGEABLE) could not land directly: under the `internal/main` ruleset's strict required-status-checks policy GitHub reports every head-`main` PR as permanently `BEHIND` (a pure mirror head can never contain the base's fork commits), so `gh pr merge` — with or without `--admin` — and direct merge-commit pushes are rejected; #138 auto-closed merged when #139 landed, as #129 did when #131 landed; `git merge-tree origin/internal/main origin/main` is clean again (zero unintegrated commits)
- fork upstream mirror: `origin/main` at `2c8e8a826` — the scheduled sync run 35597362295 fast-forwarded the mirror from `d636abd7a` to the upstream tip (2 commits: [getpaseo/paseo#5129](https://github.com/getpaseo/paseo/pull/5129) shared macOS Find shortcut, [getpaseo/paseo#4413](https://github.com/getpaseo/paseo/pull/4413) pi per-model thinking configuration), and `git merge-tree origin/internal/main origin/main` was clean (zero conflicts)
- resolver [ZGEnergy/paseo#139](https://github.com/ZGEnergy/paseo/pull/139) (two-parent merge `ade68b458`, first parent `86a86816c`, second parent `2c8e8a826`) integrated the delta with no manual resolution; focused suites on the merged tree — `find-shortcut.test.ts`, `agent-manager.test.ts` (auto-merged file), `providers/pi/agent.test.ts`, plus the capability suites `providers/omp/agent.test.ts`, `providers/omp/subagent-card-tracker.test.ts`, `providers/claude/agent.background-subagent.test.ts` — 389 tests pass; conflict resolution and nothing else, no fork product-code delta, no capability affected
- review-period fork merges since the 2026-09-20 refresh: only the #139 integration above; no new downstream product capability, nothing added or retired
- upstream: `getpaseo/paseo` `main` at `2c8e8a826` — moved 2 commits since the 2026-09-20 review, both orthogonal to the four capabilities; every capability verdict below was re-verified against this exact tree (2026-09-21 greps: zero `addMarkdownExtension`, zero `MarkdownSource`, `SvgXml` still internal icon/catalog use only, zero `optionDetails` in `providers/omp`, unbounded `providerIdleScheduler.waitForRetry()`, `appendTaskNotificationEvents` still without leftover-turn settlement) and the per-PR states re-checked via the upstream API
...
- [getpaseo/paseo#4749](https://github.com/getpaseo/paseo/pull/4749) (SvgXml, head `1a1144648baae78c4484bec7bb4a5e9252ddd0d2`), [#4750](https://github.com/getpaseo/paseo/pull/4750) (assistant markdown extensions, head `15efaa1b284aa25b86428e6fe5e5ebdfc43cf2c5`), and [#4752](https://github.com/getpaseo/paseo/pull/4752) (MarkdownSource, head `bfc1e0ecdc1a65ad3e556b72db4d16390dcc2aab`) are open and unmerged as of 2026-09-21 with unchanged heads; merge order is 4749 → 4750 → 4752.
- Upstream `main` at `2c8e8a826` contains no `addMarkdownExtension`, no plugin-facing `MarkdownSource` host component, and no host-provided `SvgXml` (re-verified 2026-09-21; zero `addMarkdownExtension`/`MarkdownSource` matches after the #5013 word-stream deletion removed even word-stream's internal streaming component, and the only `SvgXml` matches are internal icon/catalog components). Upstream #4675 (merge `0aca3b605`) moved streaming block promotion into `agent-stream/presentation.ts` and is compatible with, but does not provide, the extension API. The four new plugin commits in the prior review period — #4942 (host navigation), #4971 (host discovery with host-targeted SDK clients), #4972 (external links and workspace browsers), and #4970 (nested provider-subagent links) — extend plugin host integration but do not add the assistant-markdown extension surface.
...
- [getpaseo/paseo#3628](https://github.com/getpaseo/paseo/pull/3628) remains open (head `cd641993cd210a0af17034eec8d39f3736c8585f`, unchanged since 2026-09-14; re-checked 2026-09-21 — still the `optionDetails` decoding candidate with malformed/blank fallbacks) and still carries the description candidate.
- Upstream `main` at `2c8e8a826` retains the `16.3.9` support floor but contains no `optionDetails` decoding or propagation (re-verified 2026-09-21; zero matches in source).
...
- [getpaseo/paseo#3366](https://github.com/getpaseo/paseo/pull/3366), head `bf3820d81cc1579bc8ad4cd9721aeba972ad0b56`, remains closed unmerged (re-checked 2026-09-21, last update 2026-09-08) in favor of [#3394](https://github.com/getpaseo/paseo/pull/3394), merge `42245d139ad1f3ba93c3e691e9e0a7971169ea79`.
...
- Adjacent open upstream PRs, none merged and none carrying the missing behaviors as of 2026-09-21: [getpaseo/paseo#4594](https://github.com/getpaseo/paseo/pull/4594) (head `1549ea566e16c31d92e86bfe91b8645b80e65d5b`, unchanged since 2026-09-11; still the "spare background subagents from an interrupt, and let one be stopped" stop-control candidate, re-inspected 2026-09-20), [getpaseo/paseo#4633](https://github.com/getpaseo/paseo/pull/4633) (head `1132cdb0e6e50f4780c8c69e68c9c11e8500ef7b`, unchanged since 2026-09-10), [getpaseo/paseo#5022](https://github.com/getpaseo/paseo/pull/5022) "fix: queue follow-ups when providers cannot steer" (head `e9692f7f3`, opened 2026-09-17; app-side composer/session-store queueing of follow-ups, not provider-level admission or background-child survival), [getpaseo/paseo#5026](https://github.com/getpaseo/paseo/pull/5026) "feat(server): surface Muse subagents in the subagents track" (head `3bb630229e79657b8d90d7548c9b8c452b72ce3c`, opened 2026-09-17; a new Muse provider and track UI, not lifecycle correctness), and [getpaseo/paseo#5024](https://github.com/getpaseo/paseo/pull/5024) "fix(server): accept background on agent-scoped create_agent" (head `5d5b7f0b3e503fc1b97303d2e21f63e2b71d5b05`, opened 2026-09-17; MCP tool-schema acceptance of `background`, inspected 2026-09-21 — no admission or settlement behavior). Leftover-turn settlement still has no dedicated upstream PR.
- Upstream `main` at `2c8e8a826` (re-verified 2026-09-21) routes `task_notification` through `appendTaskNotificationEvents` (providers/claude/agent.ts), which appends provider-subagent/timeline events only and never settles a leftover autonomous turn; `steerOrReplaceActiveTurn` in packages/server/src/server/agent/agent-manager.ts (the 2026-09-17 entry wrongly cited providers/claude/agent.ts for this call site) still falls back to `replaceAdmittedForegroundTurn` when provider steering is unavailable, so follow-up admission still replaces the turn and a compatible background child neither keeps the parent admitted nor blocks replacement. The two new upstream commits this period do not touch `providers/claude`.
