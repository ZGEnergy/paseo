...
`upgrade-local` accepts only a live, attested `manager=cli` lifecycle record, so an
...
with a fork daemon normally. It won't carry fork-only app changes; the browser web UI
is the client that does.
...
`--relay true` is what lets phones and laptops reach a loopback-bound daemon. To reach
it over Tailscale or another VPN instead, set `--relay false`, bind to the tailnet
...
**Phone or laptop app:** run `paseo daemon pair` on the box for the QR code and link.
A fresh client has no host to select yet, so this is the first-connection path; the
app's own Pair device screen is for a host it already knows.
...
disables desktop auto-update because `electron-builder.yml` still names upstream as
its publish target, so a packaged fork build would otherwise replace itself with an
upstream one. See
...
Artifacts land in `packages/desktop/release/`. The build is ad-hoc signed and runs only
on the machine that produced it. Clear the quarantine attribute after installing:
